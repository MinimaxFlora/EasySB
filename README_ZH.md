...
| 复制链接 | 订阅与分享链接以卡片网格呈现在与主菜单同尺寸的固定面板里；卡片标题为订阅名（sing-box / mihomo / Base64 订阅），正文只显示 `host:port`，完整 URL 不显示。`↑`/`↓`/`←`/`→`（或数字键）选择，`Enter` 复制当前项，`C` 复制全部，`Esc` 返回；复制成功的卡片变成成功色，复制全部在页头提示。窗口变窄变矮时网格自动减少列数并截断内容，面板不溢出。日志页 `C` 复制日志（OSC52） |
