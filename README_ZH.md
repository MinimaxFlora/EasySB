...
Go 版（当前主实现）一键安装：脚本会检测系统与架构，补全运行依赖，优先下载预编译二进制（回退源码构建），并把内核一并装上：

```bash
bash <(curl -fsSL https://raw.githubusercontent.com/MinimaxFlora/EasySB/master/install.sh)
```

**内核直接编在面板里**：`go.mod` 依赖 `github.com/sagernet/sing-box`（当前 `v1.14.2`），节点就是面板自己（`sb core run -c /etc/sing-box/config.json`）。所以安装只有**一个二进制、不下载内核**，`sb` 第一次打开就能直接部署节点；流量统计也不再分来源——本仓库自己构建，`with_v2ray_api` 永远在。代价是面板体积（linux/amd64 约 60 MB）与耦合：**sing-box 版本随面板版本**，要换内核就改 `go.mod` 的依赖版本再发一版面板。
...
| 伪装站点 | 让域名看起来就是个正经服务：面板装好 OpenList / Memos / 哪吒 dashboard / Komari 之一，再用**面板自己的 443 反代**（不装 nginx）把它挂在域名下，`/sub/*` 仍归订阅服务。应用只监听 `127.0.0.1`，端口不用对外露；被探测的请求记进访问日志，一眼能看出扫描器 |
...
├── 伪装站点     用真实应用撑门面：装 / 更新 / 启停 / 改端口 / 看日志 / 卸载 OpenList、Memos、哪吒、Komari，选一个让域名指向它，并翻看访问日志
...
对应文件：服务端配置 `/etc/sing-box/config.json`，状态 `/etc/sing-box/easysb.conf`，账号 `/etc/sing-box/easysb-users.json`，伪装应用 `/opt/easysb/apps/<应用>/`（各自一个 `easysb-<应用>.service`），访问日志 `/etc/sing-box/easysb-front.log`，快捷指令 `/usr/local/bin/sb`。

---

## 伪装站点

防火墙会主动探测：它连上你的域名 443，看回的是不是一个正常站点。这一节就是把那一步做扎实——**用真实应用撑门面，而不是摆一张静态假页**。

```text
域名 us.example.com ──► 面板 443（域名证书，反代在面板进程里）
                        ├── /sub/<令牌>   订阅服务本体（客户端照旧）
                        └── 其他所有路径   选中的伪装应用（只监听 127.0.0.1）
```

| 应用 | 来源 | 形态 | 默认端口 |
| :--- | :--- | :--- | :--- |
| OpenList | `OpenListTeam/OpenList` | 单文件 Go（官方 tar.gz，校 md5），内置网盘 Web | 5244 |
| Memos | `usememos/memos` | 单文件 Go + SQLite（校 sha256） | 5230 |
| 哪吒 dashboard | `nezhahq/nezha` | zip 内单个可执行文件（上游无校验文件） | 8008 |
| Komari | `komari-monitor/komari` | 上游直接发布可执行文件 | 25774 |

命令行等价入口（与面板同一套代码，便于脚本化）：

```shell
easysb app list                     # 四个应用的状态、端口与门面设置
easysb app install openlist         # 下载（校校验）→ 装到 /opt/easysb/apps/openlist → 写单元 → 启动
easysb app logs openlist            # 应用日志（OpenList 首次登录密码就在这里）
easysb app site openlist            # 让域名指向它（需要域名 + 已签发的证书）
easysb app site off                 # 只关站点，应用与数据保留
```

要点：

- **不装 nginx**：反代就是面板进程自己（`internal/front`），证书复用面板已有的域名证书，续期后新证书下一个握手即生效。
- **没有明文入口**：伪装站点只跑 443，且没有域名证书时拒绝启用；80 只在申请 / 续期证书时被 acme.sh 临时占用。
- **应用不出网**：应用一律只监听 `127.0.0.1`，对外只有 443 上的那个站点；卸载会先列出将删除的确切路径（单元文件与 `/opt/easysb/apps/<应用>/`，含数据）并要求确认。
- **看得见探测**：「访问日志」按时间列出请求（来源 IP / Host / 方法 / 路径 / 状态码 / UA），扫描器与真人很容易区分。
...
| `core run\|check\|version` | 内核角色（子命令不是参数）：`run -c <配置>` 起节点（sing-box 服务单元执行的就是它）、`check -c <配置>` 校验配置、`version` 显示内核版本与构建标签 |
