...
安装脚本同时装内核——本仓库构建的**作者源正式版**（带流量统计），因此面板第一次打开就能直接部署节点，不用先去「内核管理」点一次。**只在本机没有内核时安装**：升级面板不会替换正在跑节点的内核；想跳过可以加 `--no-core`，想换内核则在面板 `[1] 内核管理` 里选。
...
| 内核版本管理 | 正式版 stable 与 alpha 内测版随时安装、替换、卸载，替换保留现有配置；一键安装本身已带上作者源正式版内核 |
...
| `--install-core` | 安装内核后退出（安装脚本调用）；已装同一组合则跳过，`--core-channel` / `--core-source` / `--core-force` / `--core-if-missing` 细化这次安装 |
