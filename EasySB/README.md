...
| 4. 证书管理 | 申请域名证书（standalone / webroot / DNS API）、生成自签证书、列表（含剩余天数 / 来源）、应用、删除、续期、详情、**配置证书模式（按协议切换：REALITY 换握手域名 / VMess 开关 TLS / 其余切自签↔域名证书）**、续期后自动重载 |
| 5. 伪装站点 | 部署 / 更换模板（博客 / 企业官网 / 空白页 / 自定义 HTML / 反向代理）、启用 HTTPS、关闭 |
| 6. 客户端配置与分享链接 | 全部节点链接、单节点二维码、**订阅链接**（生成 / 查看 / 二维码 / 换 token / 独立订阅站点；格式：通用 Base64 / 纯链接 / sing-box TUN 模板配置 / sing-box 精简配置 / Mihomo YAML）、客户端配置导出、关闭订阅 |
...
**Q：证书可以用自签的吗？**
可以。部署向导第 6 步会问"证书来源"：选 `申请域名证书` 走 Let's Encrypt（推荐），
选 `生成自签证书` 则现场生成 EC P-256 自签证书（不需要域名解析）。自签时脚本会自动把
`insecure` / `skip-cert-verify` 写进节点链接、客户端配置与订阅里，客户端不用手工设置。
每个协议可以单独选：菜单【证书管理】→ `9) 配置证书模式（按协议切换）`。

**Q：VMess 什么时候是 vmess-ws、什么时候是 vmess-ws-tls？**
只有"TLS 开启 + 域名证书"才叫 `vmess-ws-tls`；用自签证书或关闭 TLS 时都叫 `vmess-ws`
（链接名、Mihomo 节点名、菜单标签三处一起变）。关闭 TLS 后服务端不再需要证书。
