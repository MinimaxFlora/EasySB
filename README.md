...
| Copy links | Subscription and share-link results render as a card grid inside the same fixed panel as the main menu. Select with `↑`/`↓`/`←`/`→` (or a number key), `Enter` copies the card, `C` copies all; `Esc` returns. The full URL is never drawn. On log screens `C` copies the log (OSC52) |
