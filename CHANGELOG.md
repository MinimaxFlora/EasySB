...
- **订阅文件的下载名固定为 `EasySB`**：此前用的是账号令牌（`<token>.yaml` 等），等于把一条凭据放进下载目录，而且每个账号名字都不同；上一版改成 `EasySB.<后缀>`，但 Clash 系客户端会把文件名连后缀一起当作配置名称显示，于是显示成 `EasySB.yaml`。现在统一就是 `EasySB`（不带后缀，格式由 Content-Type 决定）。
