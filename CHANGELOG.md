...
- **工具箱**（原「服务解锁状态」）：新增 `internal/toolbox`（每个工具返回同一张表格：Headers/Rows/Notes/Summary，外加注入 Options）与 `internal/toolbox/tools` 注册表，共 14 个条目分四组——解锁检测（流媒体 / AI / 区域，就是原来的 17 项）、网络检测（三网回程、就近测速、三网测速）、IP 与端口（IP 质量、邮件端口）、硬件与性能（系统信息、硬盘信息、CPU 跑分、内存测试、磁盘 IO 顺序、磁盘 IO 4K 随机、多盘 IO）。结果改成表格显示，结论只保留「解锁 / 不解锁 / 未知」三个词（半能用记为不解锁，读不出来记未知绝不猜成解锁），地区列照抄各服务自己的判定；看板记录每项上次的结论与时间，打开分组不会自动跑任何东西。
